(function($) {
  ...
  $(function() {
    ...
    if (ace.html.hasClass('ace-tab-nav-on') > 0) {
      if (Modernizr.mq('(min-width: 992px)')) {
        ...
        if (aceOptions.nav.height !== 'auto') {
          ...
          ace.nav.obj
            .height(ace.nav.obj.height())
            .animate({ height: aceOptions.nav.height }, 700, function() {
              // Mouse Scroll
        ...
        function() {
          ...
          timer = setTimeout(function() {
            ...
            if (right + tooltip.outerWidth() < $(window).width()) {
              tooltip
                .addClass('arrow-left')
                .css({ left: right + 'px', top: top + height + 'px' });
            } else {
    ...
    $(document).on(
      'touchstart click',
      '.ace-sidebar-opened #ace-overlay',
      function(e) {
    ...
    pf_popup.open = function(el) {
      ...
      this.wrapper = $(
        '<div id="pf-popup-wrap" class="pf-popup-wrap">' +
    ...
    $(document).on(
      'touchstart click',
      '.ace-portfolio-opened #pf-popup-wrap',
      function(e) {
  ...
  function ace_header_init(head, col1, col2, col3) {
    ...
    if (
      (head.hasClass('ace-head-boxed') && head.hasClass('ace-logo-out')) ||
      (head.hasClass('ace-head-full') && col2.hasClass('text-center'))
    ) {
  ...
  function ace_is_elems_in_viewport(elems, vpart) {
    for (var i = 0; i < elems.length; i++) {
      ...
      if (
        item.hasClass('ace-animate') &&
        ace_is_elem_in_viewport(item, vpart)
      ) {
